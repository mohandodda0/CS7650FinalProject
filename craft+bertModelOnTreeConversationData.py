# -*- coding: utf-8 -*-
"""Modified (linearized without duplicates) CRAFT inference demo using ConvoKit with BERT integration

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1VlsF6_EaaGWyh0VVVXPxCH7yb7CgLdtk
...
def processDialog(voc, dialog):
    ...
    for utterance in dialog.iter_utterances():
        ...
        encoded_dict = tokenizer.encode_plus(
                        utterance.text,                    # Sentence to encode.
...
def batch2TrainData(voc, pair_batch, already_sorted=False):
    ...
    input_batch, output_batch, label_batch, id_batch, berttokens_batch, attention_mask_batch = [], [], [], [], [], []
    for pair in pair_batch:
        input_batch.append(pair[0])
        output_batch.append(pair[1])
        label_batch.append(pair[2])
        id_batch.append(pair[3])
        berttokens_batch.append(pair[4])
        attention_mask_batch.append(pair[5])
    ...
    # print(berttokens_batch)
    berttokens_batch = torch.cat(berttokens_batch, dim=0)
    attention_mask_batch = torch.cat(attention_mask_batch, dim=0)
    # berttokens_batch = torch.Tensor(berttokens_batch)
    # attention_mask_batch = torch.Tensor(attention_mask_batch)
    return inp, dialog_lengths, utt_lengths, batch_indices, dialog_indices, label_batch, id_batch, output, mask, max_target_len, berttokens_batch, attention_mask_batch
...
class SingleTargetClf(nn.Module):
    ...
    def __init__(self, hidden_size, enc_hid_dim=768, dropout=0.1):
        super(SingleTargetClf, self).__init__()
        
        self.hidden_size = hidden_size
        self.bert_hidden_dim = enc_hid_dim
        
        # initialize classifier
        self.layer1 = nn.Linear(hidden_size+enc_hid_dim, hidden_size)
    ...
    def forward(self, encoder_outputs, encoder_input_lengths, bert_output):
        ...
        # print(pooled_output.shape, last_outputs.shape)

        last_outputs = torch.cat((last_outputs,pooled_output), 1)
...
class Predictor(nn.Module):
    ...
    def __init__(self, encoder, context_encoder, classifier, transformer_model):
    ...
    def forward(self, input_batch, dialog_lengths, dialog_lengths_list, utt_lengths, batch_indices, dialog_indices, batch_size, max_length, berttokens, attention_mask):
        ...
        bert_output = self.transformer_model(berttokens, attention_mask)
        
        # Forward pass through classifier to get prediction logits
        logits = self.classifier(context_encoder_outputs, dialog_lengths, bert_output)
...
def train(input_variable, dialog_lengths, dialog_lengths_list, utt_lengths, batch_indices, dialog_indices, labels, # input/output arguments
          ...
          batch_size, clip, berttokens, attention_mask, transformer_model, transformer_optimizer, max_length=MAX_LENGTH):                                                                # misc arguments
    ...
    berttokens = berttokens.to(device)
    ...
    vals = transformer_model(berttokens, 
                          attention_mask=attention_mask)

    # Forward pass through classifier to get prediction logits
    logits = attack_clf(context_encoder_outputs, dialog_lengths, vals)
    ...
    _ =torch.nn.utils.clip_grad_norm_(transformer_model.parameters(), 1.0)
...
def validate(dataset, encoder, context_encoder, predictor, voc, batch_size, device):
    ...
    for iteration in range(1, n_iters+1):
        ...
        input_variable, dialog_lengths, utt_lengths, batch_indices, dialog_indices, labels, convo_ids, target_variable, mask, max_target_len, berttokens, attention_mask = batch
        ...
        predictions, scores = evaluateBatch(encoder, context_encoder, predictor, voc, input_variable,
                                            ...
                                            true_batch_size, device, berttokens, attention_mask)
...
def trainIters(voc, pairs, val_pairs, encoder, context_encoder, attack_clf,
               ...
               transformer_model, transformer_optimizer):
    ...
    for iteration in range(start_iteration, n_iteration + 1):
        ...
        # Extract fields from batch
        input_variable, dialog_lengths, utt_lengths, batch_indices, dialog_indices, labels, _, target_variable, mask, max_target_len, berttokens, attention_mask = training_batch
        ...
        loss = train(input_variable, dialog_lengths, dialog_lengths_list, utt_lengths, batch_indices, dialog_indices, labels, # input/output arguments
                     encoder, context_encoder, attack_clf,                                                                    # network arguments
                     encoder_optimizer, context_encoder_optimizer, attack_clf_optimizer,                                      # optimization arguments
                     true_batch_size, clip, berttokens, attention_mask, transformer_model, transformer_optimizer)        # misc arguments
                      # clip, berttokens, attention_mask, transformer_model, transformer_optimizer, transformer_scheduler
        print_loss += loss
...
def evaluateBatch(encoder, context_encoder, predictor, voc, input_batch, dialog_lengths, 
                  dialog_lengths_list, utt_lengths, batch_indices, dialog_indices, batch_size, device, berttokens, attention_mask, max_length=MAX_LENGTH):
    # Set device options
    ...
    scores = predictor(input_batch, dialog_lengths, dialog_lengths_list, utt_lengths, batch_indices, dialog_indices, batch_size, max_length, berttokens, attention_mask)
...
from transformers import get_linear_schedule_with_warmup, AutoModelForSequenceClassification, AutoModel, AdamW
...
transformer_model = AutoModel.from_pretrained(modelname)
# device = 'cpu'
# if torch.cuda.is_available():
#     device = 'cuda'+':'+str(deviceno)
transformer_model = transformer_model.to(device)
attack_clf = SingleTargetClf(hidden_size, 768, dropout)
...
trainIters(voc, train_pairs, val_pairs, encoder, context_encoder, attack_clf,
           encoder_optimizer, context_encoder_optimizer, attack_clf_optimizer, embedding,
           n_iteration, batch_size, print_every, n_iter_per_epoch, clip, transformer_model, transformer_optimizer)
